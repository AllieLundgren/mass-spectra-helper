...
if st.session_state.active_tool == "Isotope Plotter":
    ...
    if matches:
        ...
        selected_species = sorted(edited_matches.loc[edited_matches["Plot Isotopes"], "Element"].unique())

        if selected_species:
            st.subheader("Isotope Plot — Selected Species")

            sigma = st.session_state.peak_width
            colour_map_rl = {f: _COLOURS[i % len(_COLOURS)] for i, f in enumerate(selected_species)}
            fig_rl = go.Figure()
            shown_legends_rl: set[str] = set()

            for formula in selected_species:
                colour = colour_map_rl[formula]
                try:
                    df_iso = isotope_rows(formula, double_charge=False)
                except ValueError as e:
                    st.warning(f"⚠️ {e}")
                    continue
                ...
                for _, row in df_iso.iterrows():
                    show_leg = formula not in shown_legends_rl
                    if show_leg:
                        shown_legends_rl.add(formula)
