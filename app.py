...
from all_species import all_species, DEFAULT_OFF_GROUPS
...
# ═════════════════════════════════════════════════════════════════════════════
# SECTION 1b — Species index for the m/z lookup
# ═════════════════════════════════════════════════════════════════════════════
# The lookup covers tens of thousands of species, so nothing is computed up
# front except each species' composition and its lightest/heaviest possible
# mass (cheap).  A search only computes isotope patterns for the species whose
# mass range can reach the target m/z, and remembers them for later searches.
# Patterns are numpy convolutions of per-element patterns, grouped by nominal
# mass the same way molmass' spectrum() does it.

_PEAK_MIN_FRACTION   = 1e-5                  # peaks below 0.001 % are ignored
_BACKGROUND_ELEMENTS = {"O", "H", "C", "N"}  # allowed with "Only these" when ticked
_SIMPLE_FORMULA      = re.compile(r'([A-Z][a-z]?)(\d*)')
_SPECIES_KEY         = hashlib.md5(repr(all_species).encode()).hexdigest()


def _composition(formula_str: str) -> dict[str, int]:
...
def _combine_patterns(a, b):
    """Convolve two (first_mass_number, abundance, abundance × exact mass) patterns."""
    m0a, pa, wa = a
    m0b, pb, wb = b
    return (
        m0a + m0b,
        np.convolve(pa, pb),
        np.convolve(wa, pb) + np.convolve(pa, wb),
    )


def _element_pattern(symbol: str, count: int, cache: dict):
    """Nominal-mass isotope pattern of `count` atoms of `symbol`."""
    key = (symbol, count)
    if key in cache:
        return cache[key]
    if count == 1:
        ...
        m0 = min(isos)
        p  = np.zeros(max(isos) - m0 + 1)
        w  = np.zeros_like(p)
        for mn, iso in isos.items():
            p[mn - m0] += iso.abundance
            w[mn - m0] += iso.abundance * iso.mass
        total = p.sum()
        out = (m0, p / total, w / total)
    else:
        half = _element_pattern(symbol, count // 2, cache)
        out  = _combine_patterns(half, half)
        if count % 2:
            out = _combine_patterns(out, _element_pattern(symbol, 1, cache))
    cache[key] = out
    return out


def _species_peaks(comp_key: tuple, element_cache: dict):
    """(mass_numbers, mz, abundance) arrays for a sorted composition tuple."""
    out = None
    for sym, n in comp_key:
        pat = _element_pattern(sym, n, element_cache)
        out = pat if out is None else _combine_patterns(out, pat)
    m0, p, w = out
    idx = np.nonzero(p >= _PEAK_MIN_FRACTION)[0]
    return m0 + idx, w[idx] / p[idx], p[idx]


@st.cache_resource(show_spinner="Indexing species database…")
def _build_species_index(species_key: str) -> dict:
    """
    Composition and mass range of every unique species in all_species.
    Species with the same composition are kept once, under the first name/group seen.
    `species_key` only busts the cache when all_species.py changes.
    The returned dict also holds the pattern caches, which fill up as searches run.
    """
    seen: set = set()
    invalid: set = set()
    mass_range: dict[str, tuple[float, float]] = {}
    group_names = list(all_species)
    names, group_idx, keys, element_sets, lo, hi, atoms = [], [], [], [], [], [], []

    for gi, (group, formulas) in enumerate(all_species.items()):
        for formula in formulas:
            ...
            group_idx.append(gi)
            keys.append(key)
            element_sets.append(frozenset(comp))
            lo.append(sum(mass_range[s][0] * n for s, n in key))
            hi.append(sum(mass_range[s][1] * n for s, n in key))
            atoms.append(sum(comp.values()))
    ...
    return {
        ...
        "peak_cache":    {},   # composition key → (mass_numbers, mz, abundance)
        "element_cache": {},   # (symbol, count) → pattern
    }
...
def _elements_match(els: frozenset, requested: set[str], mode: str, allow_background: bool) -> bool:
    ...
    if mode == "Only these":
        allowed = (requested | _BACKGROUND_ELEMENTS) if allow_background else requested
        return els <= allowed
...
def _lookup_peaks(
    ...
    """
    Peaks within ±tol of target that pass the group/charge/abundance/element filters.
    2+ is only considered for atoms and diatomics.
    Returns (match rows sorted by error, number of candidate species checked).
    """
    group_codes = [i for i, g in enumerate(index["group_names"]) if g in groups]
    group_ok    = np.isin(index["group_idx"], group_codes)
    peak_cache  = index["peak_cache"]
    rows: list[dict] = []
    n_checked = 0

    for z in charges:
        mask = group_ok & (index["lo"] / z <= target + tol) & (index["hi"] / z >= target - tol)
        if z > 1:
            mask &= index["atoms"] <= 2
        for i in np.nonzero(mask)[0]:
            if elements and not _elements_match(index["elements"][i], elements, mode, allow_background):
                continue
            n_checked += 1
            key   = index["keys"][i]
            peaks = peak_cache.get(key)
            if peaks is None:
                peaks = peak_cache[key] = _species_peaks(key, index["element_cache"])
            mass_numbers, mz, abund = peaks
            mz_z = mz / z
            sel  = np.nonzero((np.abs(mz_z - target) <= tol) & (abund >= min_abundance))[0]
            name = index["names"][i]
            for j in sel:
                rows.append({
                    "Species":       name,
                    "Isotope":       f"{name}-{int(mass_numbers[j])}" + (" (2+)" if z == 2 else ""),
                    "Charge":        z,
                    "m/z":           float(mz_z[j]),
                    "Abundance (%)": float(abund[j]) * 100,
                    "Error":         abs(float(mz_z[j]) - target),
                    "Group":         index["group_names"][index["group_idx"][i]].replace("_", " "),
                })
...
with st.sidebar:
    ...
    if st.session_state.active_tool == "m/z Lookup":
        ...
        element_mode = st.radio(
            "Element filter mode",
            ["Any of these", "All of these", "Only these"],
            horizontal=True,
            help=(
                "**Any** — species containing at least one listed element.  \n"
                "**All** — species containing every listed element.  \n"
                "**Only** — species made from nothing but the listed elements "
                "(best when you know the sample composition)."
            ),
            key="reverse_element_mode",
        )
        allow_background = st.checkbox(
            "Also allow O, H, C, N with \"Only these\"",
            value=True,
            help="Oxygen, hydrogen, carbon and nitrogen are almost always present "
                 "(residual gas, surface contamination), so their adducts are kept.",
            disabled=element_mode != "Only these",
            key="reverse_allow_bg",
        )
        ...
        min_abundance_pct = st.number_input(
            "Min isotope abundance (%)",
            min_value=0.0,
            max_value=100.0,
            value=0.1,
            step=0.1,
            help="Hide minor isotope peaks below this abundance. Peaks under 0.001% are never stored.",
            key="reverse_min_abund",
        )
        include_2plus = st.checkbox(
            "Include 2+ ions (atoms and diatomics)",
            value=True,
            key="reverse_include_2plus",
        )
        with st.expander("Species groups"):
            selected_groups = st.multiselect(
                "Search these groups",
                options=list(all_species),
                default=[g for g in all_species if g not in DEFAULT_OFF_GROUPS],
...
if st.session_state.active_tool == "Isotope Plotter":
    ...
    # st.button() only reports True on the single rerun where it was clicked —
    # ticking a "Plot Isotopes" checkbox below triggers its own rerun where
    # run_reverse is False again, so the search results are cached in
    # session_state and only recomputed when Find Matches is actually clicked.
    species_index = _build_species_index(_SPECIES_KEY)
    st.caption(f"Database: {len(species_index['names']):,} unique species.")
    if species_index["invalid"]:
        st.warning(
            "These entries in all_species.py could not be parsed and are skipped: "
            + ", ".join(f"`{s}`" for s in species_index["invalid"])
        )
    ...
    if run_reverse and target_mz > 0:
        ...
        matches, n_checked = _lookup_peaks(
            species_index,
            target           = target_mz,
            tol              = tolerance,
            groups           = set(selected_groups),
            charges          = (1, 2) if include_2plus else (1,),
            min_abundance    = min_abundance_pct / 100,
            elements         = requested_elements,
            mode             = element_mode,
            allow_background = allow_background,
        )
        st.session_state.reverse_matches = matches
        st.session_state.reverse_matches_ran = True
        st.session_state.reverse_search_info = (
            f"Checked {n_checked:,} candidate species in {time.perf_counter() - _t0:.2f} s."
        )

    if st.session_state.get("reverse_search_info"):
        st.caption(st.session_state.reverse_search_info)

    matches = st.session_state.get("reverse_matches", [])

    if matches:
        match_df = pd.DataFrame(matches).sort_values("Error").reset_index(drop=True)
        match_df["Plot Isotopes"] = False
        ...
        edited_matches = st.data_editor(
            ...
            column_config={
                ...
                "Isotope":        st.column_config.TextColumn("Isotope",         disabled=True),
                "Charge":         st.column_config.NumberColumn("Charge",        disabled=True, format="%d"),
                "m/z":            st.column_config.NumberColumn("m/z",           disabled=True, format="%.5f"),
                "Abundance (%)":  st.column_config.NumberColumn("Abundance (%)", disabled=True, format="%.3f"),
                "Error":          st.column_config.NumberColumn("Error",         disabled=True, format="%.5f"),
                "Group":          st.column_config.TextColumn("Group",           disabled=True),
                "Plot Isotopes":  st.column_config.CheckboxColumn("Plot Isotopes"),
            },
            hide_index=True,
            width="stretch",
            key="reverse_lookup_editor",
        )

        # (species, charge) pairs so a 2+ match plots its 2+ pattern
        selected_species = sorted(
            edited_matches.loc[edited_matches["Plot Isotopes"], ["Species", "Charge"]]
            .drop_duplicates()
            .itertuples(index=False, name=None)
        )

        if selected_species:
            st.subheader("Isotope Plot — Selected Species")

            sigma = st.session_state.peak_width
            colour_map_rl = {sel: _COLOURS[i % len(_COLOURS)] for i, sel in enumerate(selected_species)}
            fig_rl = go.Figure()
            shown_legends_rl: set[str] = set()

            for formula, charge in selected_species:
                colour = colour_map_rl[(formula, charge)]
                name   = formula + (" (2+)" if charge == 2 else "")
                try:
                    df_iso = isotope_rows(formula, double_charge=charge == 2)
                except ValueError as e:
                    st.warning(f"⚠️ {e}")
                    continue

                max_ri = df_iso["relative_intensity"].max()
                for _, row in df_iso.iterrows():
                    show_leg = name not in shown_legends_rl
                    if show_leg:
                        shown_legends_rl.add(name)

                    amp = (row["relative_intensity"] / max_ri) * 0.5 if max_ri > 0 else 0.5
                    fig_rl.add_trace(_make_gaussian_trace(
                        centre      = row["mz"],
                        amplitude   = amp,
                        sigma       = sigma,
                        colour      = colour,
                        ...
                        showlegend  = show_leg,
                        legendgroup = name,
                        hover_text  = f"Label: {row['label']}<br>Abundance: {row['abundance'] * 100:.3f}%",
                    ))
            ...
            fig_rl.update_xaxes(showgrid=True, gridcolor="rgba(128,128,128,0.2)", zeroline=False, showline=True, linewidth=1, linecolor="gray", mirror=True)
            fig_rl.update_yaxes(showgrid=True, gridcolor="rgba(128,128,128,0.2)", zeroline=False, showline=True, linewidth=1, linecolor="gray", mirror=True)
            st.plotly_chart(fig_rl)
    elif st.session_state.get("reverse_matches_ran"):
        st.warning("No matches found within tolerance.")
    else:
        st.info("Enter an m/z value and click **Find Matches** to search.")
    ...
    st.divider()
    st.header("Element → Species Weight % Converter")
    st.caption(
        "Convert an elemental weight percent into the equivalent weight percent of a "
        "chosen species, assuming all of the element is present as that species "
        "(e.g. 20 wt% Zr → wt% as ZrO2)."
    )

    conv_col1, conv_col2, conv_col3 = st.columns(3)
    with conv_col1:
        conv_element = st.text_input("Element", "", placeholder="e.g. Zr", key="e2s_element")
    with conv_col2:
        conv_wt_pct = st.number_input(
            "Elemental Weight %", min_value=0.0, value=0.0, step=0.1, key="e2s_wt_pct"
        )
    with conv_col3:
        conv_species = st.text_input("Desired Species", "", placeholder="e.g. ZrO2", key="e2s_species")

    if st.button("Convert", key="e2s_convert_btn"):
        try:
            element_sym = conv_element.strip().capitalize()
            species_formula = Formula(conv_species.strip())
            composition = species_formula.composition()

            if element_sym not in composition:
                st.error(f"**{element_sym}** was not found in **{conv_species.strip()}**.")
            else:
                item = composition[element_sym]
                mass_fraction = item.mass / species_formula.mass
                species_wt_pct = conv_wt_pct / mass_fraction
                st.success(
                    f"**{conv_wt_pct:g} wt% {element_sym}** → "
                    f"**{species_wt_pct:.4f} wt% {conv_species.strip()}**"
                )
        except Exception as e:
            st.error(f"Conversion failed: {e}")
