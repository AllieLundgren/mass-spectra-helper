...
# ═════════════════════════════════════════════════════════════════════════════
# SECTION 1b — Precomputed species peak table for the m/z lookup
# ═════════════════════════════════════════════════════════════════════════════
# The lookup searches tens of thousands of species, so every isotope peak is
# computed once (numpy convolution of per-element patterns, same nominal-mass
# grouping as molmass' spectrum()) and cached; each search is then a binary
# search on the sorted m/z column.

_PEAK_MIN_FRACTION   = 1e-5                  # peaks below 0.001 % are not stored
...
def _element_pattern(symbol: str, count: int):
    ...
    if count == 1:
        ...
        total = p.sum()
        return m0, p / total, w / total
    half = _element_pattern(symbol, count // 2)
    out  = _combine_patterns(half, half)
    if count % 2:
        out = _combine_patterns(out, _element_pattern(symbol, 1))
    return out


def _species_pattern(comp: dict[str, int]):
    out = None
...
@st.cache_resource(show_spinner="Building species peak table (first run only)…")
def _build_peak_table(species_key: str):
    """
    One row per isotope peak of every species in all_species, sorted by m/z.
    Species with the same composition are kept once, under the first name/group seen.
    Atoms and diatomics also get 2+ rows.  `species_key` only busts the cache
    when all_species.py changes.

    Returns (table, number_of_species, invalid_formulas).
    """
    seen: set = set()
    invalid: set = set()
    species, groups, element_sets = [], [], []
    mass_numbers, mzs, abundances, charges = [], [], [], []
    ...
    for group, formulas in all_species.items():
        for formula in formulas:
            ...
            mz  = w[idx] / p[idx]
            els = frozenset(comp)
            for z in ((1, 2) if sum(comp.values()) <= 2 else (1,)):
                species.extend([formula] * n)
                groups.extend([group] * n)
                element_sets.extend([els] * n)
                mass_numbers.append(m0 + idx)
                mzs.append(mz / z)
                abundances.append(p[idx])
                charges.append(np.full(n, z))

    table = pd.DataFrame({
        "species":     species,
        "group":       pd.Categorical(groups),
        "elements":    element_sets,
        "mass_number": np.concatenate(mass_numbers),
        "mz":          np.concatenate(mzs),
        "abundance":   np.concatenate(abundances),
        "charge":      np.concatenate(charges),
    })
    table = table.sort_values("mz", kind="stable").reset_index(drop=True)
    return table, len(seen), sorted(invalid)
...
def _lookup_peaks(
    ...
    lo   = np.searchsorted(mz, target - tol, side="left")
    hi   = np.searchsorted(mz, target + tol, side="right")
    hits = table.iloc[lo:hi]
    hits = hits[
        hits["group"].isin(groups)
        & hits["charge"].isin(charges)
        & (hits["abundance"] >= min_abundance)
    ]
    if elements:
        if mode == "All of these":
            keep = hits["elements"].map(lambda els: elements <= els)
        elif mode == "Only these":
            allowed = (elements | _BACKGROUND_ELEMENTS) if allow_background else elements
            keep = hits["elements"].map(lambda els: els <= allowed)
        else:  # "Any of these"
            keep = hits["elements"].map(lambda els: bool(elements & els))
        hits = hits[keep.astype(bool)]
    hits = hits.copy()
    hits["error"] = (hits["mz"] - target).abs()
    return hits.sort_values("error")
...
if st.session_state.active_tool == "Isotope Plotter":
    ...
    peak_table, n_species, invalid_species = _build_peak_table(_SPECIES_KEY)
    st.caption(f"Database: {n_species:,} unique species, {len(peak_table):,} isotope peaks.")
    if invalid_species:
        st.warning(
            "These entries in all_species.py could not be parsed and are skipped: "
            + ", ".join(f"`{s}`" for s in invalid_species)
        )
    ...
    if run_reverse and target_mz > 0:
        ...
        st.session_state.reverse_matches = [
            {
                "Species":       r.species,
                "Isotope":       f"{r.species}-{r.mass_number}" + (" (2+)" if r.charge == 2 else ""),
                "Charge":        int(r.charge),
                "m/z":           float(r.mz),
                "Abundance (%)": float(r.abundance) * 100,
                "Error":         float(r.error),
                "Group":         str(r.group).replace("_", " "),
            }
